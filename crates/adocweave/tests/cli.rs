...
fn cli_reports_machine_readable_release_contracts() {
    ...
    assert_eq!(value["name"], "adocweave");
...
fn include_check_projects_diagnostics_to_the_resource_file() {
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("adocweave-{unique}"));
    std::fs::create_dir_all(&root).expect("directory");
    let document = root.join("root.adoc");
...
fn include_provider_rejects_a_symlink_escape() {
    ...
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("adocweave-root-{unique}"));
    let outside = std::env::temp_dir().join(format!("adocweave-outside-{unique}.adoc"));
    std::fs::create_dir_all(&root).expect("directory");
...
fn include_diagnostics_use_logical_ids_for_canonical_file_aliases() {
    ...
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("adocweave-alias-{unique}"));
    std::fs::create_dir_all(&root).expect("directory");
    let document = root.join("root.adoc");
    std::fs::write(&document, "include::part.adoc[]\ninclude::alias.adoc[]\n")
        .expect("root source");
...
fn complete_conversion_embeds_validated_stylesheets_in_order() {
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let css = std::env::temp_dir().join(format!("adocweave-css-{unique}.css"));
...
fn stylesheet_options_fail_closed() {
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let evil = std::env::temp_dir().join(format!("adocweave-evil-{unique}.css"));
