use adocweave_core::semantic::{Block, VerbatimKind};
use adocweave_core::semantic::{Inline, InlineLiteralKind, InlineStyle};
use adocweave_core::text::{SyntaxIssueClass, SyntaxKind};
...
fn grammar_ambiguous_fixture_has_normative_ast_and_recovery() {
    ...
    let diagnostics = adocweave_core::output::diagnostics::lint_analysis(
        &parsed,
    ...
    adocweave_core::output::diagnostics::sort_diagnostics(&mut sorted_diagnostics);
    assert_eq!(
