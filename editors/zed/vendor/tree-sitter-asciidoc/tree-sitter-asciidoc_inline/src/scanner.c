...
// Scan an unconstrained monospace span, including both `` delimiters.
// A backslash immediately before the closing delimiter is content; it does
// not escape the first backtick of that delimiter. A single backtick after a
// backslash remains an escaped content character.
static bool scan_unconstrained_monospace_content(TSLexer *lexer) {
    while (!lexer->eof(lexer) && lexer->lookahead != '\n' &&
        ...
        if (lexer->lookahead == '\\') {
            ...
            lexer->result_symbol = TOKEN_UNCONSTRAINED_MONOSPACE;
            return true;
        }
        if (lexer->lookahead == '`') {
            lexer->advance(lexer, false);
...
static bool scan_monospace_begin(TSLexer *lexer, bool constrained_valid,
                                 bool unconstrained_valid) {
    lexer->advance(lexer, false);
    int32_t after = lexer->lookahead;
    if (after == '`') {
        if (!unconstrained_valid) {
            return false;
        }
        lexer->advance(lexer, false);
        return scan_unconstrained_monospace_content(lexer);
    }
    if (!constrained_valid || lexer->eof(lexer) || is_space(after) ||
        after == '\n' || after == '\r') {
        return false;
    }
    lexer->mark_end(lexer);
    ...
    while (!lexer->eof(lexer) && lexer->lookahead != '\n' &&
        ...
        if (c == '`' && prev != '\\' && !is_space(prev)) {
            lexer->advance(lexer, false);
            int32_t next = lexer->lookahead;
            if (next != '`' && (lexer->eof(lexer) || !is_word_char(next))) {
                lexer->result_symbol = TOKEN_MONOSPACE_BEGIN;
                return true;
            }
            prev = c;
            continue;
...
bool tree_sitter_asciidoc_inline_external_scanner_scan(
    ...
    if ((valid_symbols[TOKEN_MONOSPACE_BEGIN] ||
         valid_symbols[TOKEN_UNCONSTRAINED_MONOSPACE]) &&
        la == '`') {
        return scan_monospace_begin(
            lexer, valid_symbols[TOKEN_MONOSPACE_BEGIN],
            valid_symbols[TOKEN_UNCONSTRAINED_MONOSPACE]);
    }
