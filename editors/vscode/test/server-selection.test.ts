...
test("明示した絶対pathをPATHより先に選択します", async () => {
  let pathSearches = 0;
  const configuredPath = resolve("explicit-adocweave");
  const selected = await selectServer(
    { configuredPath, storageDirectory },
    dependencies({
      findOnPath: async () => {
        pathSearches += 1;
        return resolve("path-adocweave");
      },
    }),
  );
...
test("設定がない場合はPATH上の絶対pathを選択します", async () => {
  const pathCandidate = resolve("path-adocweave");
  assert.deepEqual(
...
test("相対設定pathは取得へ倒さず拒否します", async () => {
  await assert.rejects(
    selectServer({ configuredPath: "relative/adocweave", storageDirectory }, dependencies()),
    /configured-server-path-not-absolute/,
  );
});

test("PATH上の相対候補は採用せず、取得へ進みます", async () => {
  const downloaded = resolve("storage", "adocweave");
  assert.deepEqual(
    await selectServer(
      options,
      dependencies({
        findOnPath: async () => "relative/adocweave",
...
test("どちらにも見つからない場合だけ自動取得へ進みます", async () => {
  const downloaded = resolve("storage", "adocweave-0.47.0-target", "adocweave");
