...
test(`Release Notesはv${RELEASE_NOTES_VERSION}の変更内容と移行方法を含む`, () => {
  ...
  assert.equal(PREVIOUS_RELEASE_VERSION, "0.41.0");
  ...
  assert.match(notes, /footnote.*本文.*inline/);
  assert.match(notes, /keishis\.cachix\.org/);
  assert.match(notes, /WASM protocol schema version/);
  assert.match(notes, /どちらもv0\.41\.0から変更していません/);
