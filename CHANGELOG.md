...
- Download the native executable from the `v0.56.1` Release instead of the former `adocweave-cli/vX.Y.Z` and `adocweave-lsp/vX.Y.Z` Releases. Each platform archive now contains only `adocweave`.
- Pin Rust workspace APIs with `v0.56.1` instead of the former `adocweave-lib/vX.Y.Z` tag.
...
[0.56.1]: https://github.com/KeishiS/adocweave/releases/tag/v0.56.1
